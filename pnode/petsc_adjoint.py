...
class ODEPetsc(object):
    ...
    def evalIJacobian(self, ts, t, U, Udot, shift, Jac, JacPre):
        ...
        if self.innerkspmat is not None and self.reset_jacobianIM:
            ...
            if self.fixed_jacobian_across_steps: # resue the jacobian across steps
                self.reset_jacobianIM = False
    ...
    def setupTS(
        ...
        fixed_jacobian_across_steps=False,
    ):
        """
        ...
        Args:
            ...
            matrixfree_jacobian: Specifies whether to use matrix-free Jacobian (for implicit time integration methods). If not, explicit Jacobians are assembled by using functorch's jacrev() at the beginning of each ODE solver and they are assumed to be constant during the entire time integration process. By default matrixfree is True. This option could be ignored depending on the settings of linear_solver.
            fixed_jacobian_across_solves: Specifies if the non matrix-free IJacobian is constant across ODE solves. If true, fixed_jacobian_across_steps is also set to true regardless of user input. Have no effect when matrix-free IJacobian is used.
            fixed_jacobian_across_steps: Specifies if the non matrix-free IJacobian is constant across time steps inside an ODE solve. Have no effect when matrix-free IJacobian is used.
        """
        ...
        if fixed_jacobian_across_solves == True:
            self.fixed_jacobian_across_steps = True
        else:
            self.fixed_jacobian_across_steps = fixed_jacobian_across_steps
        ...
        if not matrixfree_jacobian: # begins by computing the Jacobian
        ...
        if (
            ...
            if implicit_form or imex_form:
                ...
                if self.linear_solver != "petsc":
                    ...
                    ksp = snes.getKSP()
                    pc = PETSc.PC()
                    always_update_jacobian = not self.fixed_jacobian_across_solves and not self.fixed_jacobian_across_steps
                    pcshell = PCShell(
                        innerkspmat, batch_size, n // batch_size, self.use_cuda, always_update_jacobian
                    )
    ...
    def odeint(self, u0, t):
        ...
        """
        if not self.fixed_jacobian_across_solves and not self.matrixfree_jacobian:
        ...
        if self.fixed_jacobian_across_solves and not self.matrixfree_jacobian:
            self.reset_jacobianIM==False
        return solution
